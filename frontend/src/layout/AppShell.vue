...
const aiPageContext = computed(() => {
  ...
  const plans: Record<string, { section: string; data_domains: string[]; datasets: string[]; page_goal: string; recommended_questions: string[] }> = {
    ...
    "marketing-new-customer": { section: "新客折扣", data_domains: ["campaign", "customer"], datasets: ["new_customer_discount", "store_overview", "customers", "products"], page_goal: "评估新客折扣带来的新客成交和店铺增量信号", recommended_questions: ["新客折扣带来的成交质量怎么样？", "新客折扣转化下降原因是什么？", "这个活动应该继续吗？"] },
