...
function statusLabel(status: "complete" | "partial" | "missing"): string { return status === "partial" ? "部分字段" : status === "missing" ? "未采集" : "" }
...
<template>
  <template v-if="dashboard && value">
    ...
    <section class="decision-chart-grid"><article class="panel"><div class="panel-heading"><div><p>新客趋势</p><h2>触达、支付与成交金额</h2></div><span v-if="missingDays.length || partialDays.length" class="panel-action">有效 {{ completeDays.length }} / {{ value.daily_metrics.length }} 天</span></div><BusinessChart :option="trendOption" ariaLabel="新客折扣新访客和成交趋势图" :height="340" /></article><article class="panel"><div class="panel-heading"><div><p>贡献对照</p><h2>新客折扣对全店新客的贡献</h2></div></div><BusinessChart :option="compareOption" ariaLabel="新客折扣支付金额和买家贡献占比图" :height="340" /></article></section>
...
<style scoped>
.new-customer-coverage-warning { display: flex; align-items: flex-start; gap: 10px; border: 1px solid #efd9a8; border-radius: 6px; padding: 13px 15px; color: #a16b18; background: #fffaf0; }.new-customer-coverage-warning > svg { flex: 0 0 auto; margin-top: 2px; }.new-customer-coverage-warning div { display: grid; gap: 4px; }.new-customer-coverage-warning strong { color: #76511b; font-size: 12px; }.new-customer-coverage-warning span, .new-customer-coverage-warning small { color: #9a762f; font-size: 12px; line-height: 1.55; }.new-customer-coverage-warning small { color: #8b8069; }
.new-customer-daily-panel { min-width: 0; overflow: hidden; }.new-customer-daily-table { margin-top: 12px; overflow-x: auto; }.new-customer-daily-row { display: grid; min-width: 940px; grid-template-columns: 1.05fr repeat(7, .9fr); min-height: 44px; align-items: center; gap: 10px; border-bottom: 1px solid #edf2ef; color: #687970; font-size: 12px; text-align: right; }.new-customer-daily-row > :first-child { text-align: left; }.new-customer-daily-row > div { display: grid; gap: 3px; }.new-customer-daily-row strong, .new-customer-daily-row b { color: #40564b; font-weight: 650; }.new-customer-daily-row small { color: #98a49d; font-size: 12px; }.new-customer-daily-row em { color: #177a55; font-style: normal; font-weight: 650; }.new-customer-daily-row.is-partial { background: #fffaf0; }.new-customer-daily-row.is-missing { color: #a39070; background: #fcfaf6; }.new-customer-daily-head { min-height: 32px; color: #98a49d; font-size: 12px; }.new-customer-daily-head > :not(:first-child) { text-align: right; }
</style>
