...
const rankingVisitorFloor = computed(() => Math.max(100, Math.ceil(totalVisitors.value * 0.01)))
const comparableSources = computed(() => trafficTree.value.filter((item) => item.visitors >= rankingVisitorFloor.value && item.buyers > 0))

const largestTrafficSource = computed(() => [...trafficTree.value].sort((left, right) => right.visitors - left.visitors)[0])
const highestConversionSource = computed(() => [...comparableSources.value].sort((left, right) => right.conversion_rate - left.conversion_rate)[0])
const highestNewVisitorSource = computed(() => [...comparableSources.value].sort((left, right) => newVisitorShare(right) - newVisitorShare(left))[0])
const highestUvSource = computed(() => [...comparableSources.value].sort((left, right) => right.uv_value - left.uv_value)[0])
...
const qualityOption = computed(() => ({
  ...
  xAxis: { type: "value", name: "新访客率", nameLocation: "middle", nameGap: 38, min: 0, max: 100, splitNumber: 5, nameTextStyle: { color: "#718078", fontSize: 12, fontWeight: 600 }, axisLabel: { color: "#7c8982", fontSize: 12, formatter: "{value}%" }, axisLine: { lineStyle: { color: "#b3beb8" } }, axisTick: { show: false }, splitLine: { lineStyle: { color: "#edf1ef" } } },
  yAxis: { type: "value", name: "支付转化率", nameLocation: "middle", nameGap: 48, min: 0, nameTextStyle: { color: "#718078", fontSize: 12, fontWeight: 600 }, axisLabel: { color: "#7c8982", fontSize: 12, formatter: "{value}%" }, axisLine: { lineStyle: { color: "#b3beb8" } }, axisTick: { show: false }, splitLine: { lineStyle: { color: "#edf1ef" } } },
  series: [{
    type: "scatter",
    ...
    label: { show: true, position: "top", color: "#50635a", fontSize: 12, formatter: (params: { data: [number, number, number, string] }) => params.data[3] },
...
async function loadTree(): Promise<void> {
  ...
  try {
    ...
    trafficTree.value = await fetchTrafficTree(dashboard.value.range_start, dashboard.value.range_end, currentStoreId.value ?? stores[0]?.store_id)
...
<template>
  <template v-if="dashboard">
    <section class="traffic-context-strip"><div><Route :size="16" /><strong>流量来源</strong><span>按平台来源树比较规模与效率，来源之间可能存在归因重叠。</span></div><em>{{ trafficTree.length }} 个一级来源 · {{ flattenedNodes.length }} 个来源节点</em></section>

    <section class="metrics-grid traffic-quality-metrics">
      <MetricCard label="来源访客人次" :value="number(totalVisitors)" detail="一级来源求和，跨来源可能重复" :icon="Eye" tone="blue" scope="来源汇总" />
      <MetricCard label="来源新访客率" :value="ratio(newVisitorRate)" :detail="`${number(totalNewVisitors)} 新访客人次`" :icon="UserPlus" tone="teal" scope="加权口径" />
      <MetricCard label="来源支付转化" :value="ratio(sourceConversionRate)" :detail="`${number(totalBuyers)} 支付买家人次`" :icon="MousePointerClick" tone="amber" scope="加权口径" />
      <MetricCard label="来源 UV 价值" :value="currency(sourceUvValue)" detail="一级归因金额 / 来源访客人次" :icon="Target" tone="coral" scope="加权口径" />
    </section>
...
@media (max-width: 760px) {
  ...
  .traffic-source-row:not(.traffic-source-head) > :nth-child(2)::before { content: "访客 "; color: #98a39d; font-size: 12px; font-weight: 400; }
  .traffic-source-row:not(.traffic-source-head) > :nth-child(3)::before { content: "买家 "; color: #98a39d; font-size: 12px; font-weight: 400; }
  .traffic-source-row:not(.traffic-source-head) > :nth-child(4)::before { content: "转化 "; color: #98a39d; font-size: 12px; font-weight: 400; }
  .traffic-source-row:not(.traffic-source-head) > :nth-child(5)::before { content: "成交 "; color: #98a39d; font-size: 12px; font-weight: 400; }
