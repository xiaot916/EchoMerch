...
async function collect(datasetNames?: string[], refreshExisting = false): Promise<void> {
  ...
  try {
    ...
    await startDailyCollection({ day: selectedDay.value, datasetNames, sessionSource: "drissionpage", refreshExisting })
    actionNotice.value = "所需平台登录状态已校验，采集任务已启动。"
    await load()
  } catch (requestError) {
...
function statusDetail(item: DatasetCoverage): string {
  ...
  if (item.status === "failed") return item.error_message || "最近一次采集失败"
  if (item.status === "collecting") return "后台 Worker 正在处理"
  return item.error_message || (item.latest_date ? `最近数据停在 ${item.latest_date}` : "尚未采集到数据")
}

function tableStatusDetail(item: DatasetCoverage): string {
  const noDataTables = item.tables.filter((table) => table.status === "no_data")
  if (!noDataTables.length) return `${item.present_tables}/${item.expected_tables} 张表已处理`
  const labels = noDataTables.map((table) => table.table.includes("sample") ? "派样" : table.table.includes("repurchase") ? "复购" : table.table).join("、")
  return `${item.present_tables}/${item.expected_tables} 张表已处理 · ${labels}平台无数据`
}
...
<template>
  <section class="module-hero collection-command-hero">
    ...
    <div class="hero-actions">
      ...
      <button class="capture-refresh" :disabled="!canManage || starting || running || !attentionDatasets.length" title="采集缺失或异常的数据集" @click="collectAttention"><Play :size="16" />{{ running ? "采集中" : `补采异常项 ${attentionDatasets.length}` }}</button>
    </div>
  </section>
  ...
  <template v-else-if="overview">
    <div v-if="actionError" class="collection-alert"><AlertTriangle :size="16" /><span>{{ actionError }}</span></div>
    <div v-if="actionNotice" class="collection-alert collection-context-note"><CircleAlert :size="16" /><span>{{ actionNotice }}</span></div>
    ...
    <section class="collection-workspace-grid">
      <article class="panel collection-dataset-panel">
        ...
        <div class="collection-dataset-table"><div class="collection-dataset-head"><span>数据集</span><span>状态</span><span>表级到达</span><span>最近数据</span><span>操作</span></div><div v-for="item in visibleDatasets" :key="item.key" class="collection-dataset-row"><div class="dataset-name-cell"><strong>{{ item.label }}</strong><span>{{ item.group }} · {{ item.description }}</span><small v-if="isDailyPromotionDetail(item)" class="dataset-mode-badge">每日批量明细 · 按业务日期逐日留存，不覆盖历史</small><small v-else-if="item.collection_mode === 'coverage_snapshot'" class="dataset-mode-badge">每日全量覆盖 · 在线商品、当前价与红线价覆盖当天完整集合</small><small v-if="item.error_message">{{ item.error_message }}</small></div><div><span class="dataset-status" :class="item.status">{{ statusLabel(item) }}</span><small>{{ statusDetail(item) }}</small></div><div class="table-arrival-cell"><strong>{{ tableStatusDetail(item) }}</strong><span v-for="table in item.tables" :key="table.table" :title="table.table"><Check v-if="table.present" :size="12" /><CircleAlert v-else :size="12" />{{ table.row_count }} 行<span v-if="table.status === 'no_data'"> · 平台无数据</span></span></div><div><strong>{{ item.latest_date || "--" }}</strong><small>{{ item.last_attempt_at ? `尝试 ${formatTime(item.last_attempt_at)}` : "无采集记录" }}</small></div><div class="dataset-action-cell"><button :disabled="!canManage || starting || running || item.status === 'no_data'" :title="item.status === 'complete' ? '重新采集此数据集' : item.status === 'partial' ? '重新执行并补齐缺失表' : item.status === 'no_data' ? '平台已明确返回无数据，无需补采' : '补采此数据集'" @click="collect([item.key], ['complete', 'partial'].includes(item.status))"><RotateCcw :size="15" />{{ item.status === "complete" ? "重采" : item.status === "partial" ? "补齐" : item.status === "no_data" ? "无需补采" : "补采" }}</button></div></div></div>
      </article>
