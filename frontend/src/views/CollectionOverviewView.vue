...
const isHistoricalSnapshot = (key: string): boolean => key === "taobao_operational_snapshots" && selectedDay.value !== shanghaiToday()
const actionableDatasets = computed(() => attentionDatasets.value.filter((item) => !isHistoricalSnapshot(item.key)))
...
async function collect(datasetNames?: string[], refreshExisting = false): Promise<void> {
  ...
  if (requestedDatasets.some(isHistoricalSnapshot)) {
    actionError.value = "淘宝运营商品快照只提供当天状态，历史日期无法补采；请选择其他数据集或查看当天快照。"
    return
  }
...
async function collectAttention(): Promise<void> {
  ...
  await collect(actionableDatasets.value.map((item) => item.key), true)
}
...
function statusLabel(item: DatasetCoverage): string {
  if (item.status === "no_data") return item.key === "sycm_activity_calendar" ? "无活动" : item.collection_mode === "coverage_snapshot" ? "无在线商品" : "平台无数据"
...
function statusDetail(item: DatasetCoverage): string {
  if (isHistoricalSnapshot(item.key) && !["complete", "no_data"].includes(item.status)) {
    return `${selectedDay.value} 快照未留存；平台只返回当前在线状态，历史日期无法补采。${item.error_message ? `原始错误：${item.error_message}` : ""}`
  }
  ...
  if (item.status === "no_data") return item.key === "sycm_activity_calendar" ? "覆盖完成 · 当前没有活动" : item.collection_mode === "coverage_snapshot" ? "覆盖完成 · 当天没有在线商品" : "接口已返回，目标日期无业务数据"
...
<template>
  <section class="module-hero collection-command-hero">
    ...
    <div class="hero-actions">
      ...
      <button class="capture-refresh" :disabled="!canManage || starting || running || !actionableDatasets.length" title="补采可追溯的缺失数据；历史运营商品快照无法补采" @click="collectAttention"><LoaderCircle v-if="starting" :size="16" class="spinning" /><Play v-else :size="16" />{{ starting ? "正在准备补采" : running ? "采集中" : `补采异常项 ${actionableDatasets.length}` }}</button>
    </div>
  </section>
