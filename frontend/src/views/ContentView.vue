...
const funnelOption = computed(() => ({
  ...
  series: [{ type: "funnel", left: "8%", top: 20, bottom: 20, width: "84%", min: 0, max: Math.max(...funnelStages.value.map((item) => item.value), 1), minSize: "14%", maxSize: "100%", sort: "none", gap: 3, label: { show: true, position: "inside", color: "#264235", fontSize: 12, formatter: (params: { name: string; value: number; data: { conversion: number | null } }) => `${params.name} ${number(params.value)}${params.data.conversion == null ? "" : `\n${ratio(params.data.conversion)}`}` }, data: funnelStages.value }],
}))
...
<style scoped>
.content-daily-panel { min-width: 0; overflow: hidden; }.content-daily-table { margin-top: 12px; overflow-x: auto; }.content-daily-row { display: grid; min-width: 850px; grid-template-columns: 1fr .8fr 1fr 1fr 1.1fr 1fr .9fr; min-height: 44px; align-items: center; gap: 11px; border-bottom: 1px solid #edf2ef; color: #687a70; font-size: 12px; text-align: right; }.content-daily-row > :first-child { text-align: left; }.content-daily-row > div { display: grid; gap: 2px; }.content-daily-row strong, .content-daily-row b { color: #40564b; font-weight: 650; }.content-daily-row small { color: #98a59e; font-size: 12px; }.content-daily-row em { color: #167b55; font-style: normal; font-weight: 700; }.content-daily-head { min-height: 32px; color: #98a59e; font-size: 12px; }.content-daily-head > :not(:first-child) { text-align: right; }
</style>
