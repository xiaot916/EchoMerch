...
const promotionActionRows = computed<BusinessActionRow[]>(() => {
  ...
  for (const item of pauseCandidates.slice(0, 3)) rows.push({ id: `pause-${item.dimension_id}`, priority: "P0", object: item.dimension_name, issue: "暂停扩量并优先排查", evidence: `${item.scene_name} · 花费 ${currency(item.spend)} · ROI ${item.roi.toFixed(2)}x · 点击成交 ${ratio(item.cvr)}`, impact: currency(item.spend), action: "冻结新增预算，依次下钻单元、关键词/人群和承接商品；保留最小验证量。", validation: "归因成交、点击成交率、直接/间接成交", window: "1-3 天", tone: "risk" })
  ...
  const trafficCandidates = campaignMetrics.value.filter((item) => item.clicks >= 100 && item.cvr < Math.max((summary.value?.click_conversion_rate ?? 0) * .6, .5)).sort((left, right) => right.clicks - left.clicks)
  for (const item of trafficCandidates.slice(0, 2)) rows.push({ id: `traffic-${item.dimension_id}`, priority: "P1", object: item.dimension_name, issue: "高点击低成交，先修承接", evidence: `${number(item.clicks)} 点击 · CTR ${ratio(item.ctr)} · 点击成交 ${ratio(item.cvr)}`, impact: currency(item.spend), action: "核对搜索词/人群与商品匹配，检查详情、价格、评价和库存后再决定是否降价扩量。", validation: "点击成交率、加购率、支付买家", window: "3-7 天", tone: "warning" })
  const scaleCandidates = campaignMetrics.value.filter((item) => item.roi >= 4 && item.spend >= 500 && item.buyers >= 3).sort((left, right) => right.paid_amount - left.paid_amount)
