...
import { AlertTriangle, ChevronLeft, ChevronRight, CheckCircle2, Clock3, Database, LoaderCircle, PackageSearch, RefreshCw, Search } from "lucide-vue-next"
...
const overviewData = ref<InventoryManagementResponse | null>(null)
const baseAnalysis = ref<InventoryAnalysisResponse | null>(null)
const companyAnalysis = ref<InventoryAnalysisResponse | null>(null)
const packageAnalysis = ref<InventoryAnalysisResponse | null>(null)
const analysis = computed(() => activeView.value === "company" ? companyAnalysis.value : packageAnalysis.value)
...
const storePanel = ref<HTMLElement | null>(null)
const analysisQuery = ref("")
const analysisStatus = ref("")
const packageQuery = ref("")
const appliedAnalysisQuery = ref("")
const appliedAnalysisStatus = ref("")
const appliedPackageQuery = ref("")
...
const hasAppliedStoreFilters = computed(() => Boolean(appliedStoreFilters.value.query || appliedStoreFilters.value.series || appliedStoreFilters.value.specification || appliedStoreFilters.value.size || appliedStoreFilters.value.stockStatus))
const appliedStoreFilters = ref({ query: "", series: "", specification: "", size: "", stockStatus: "" })
const storeResultLabel = computed(() => hasAppliedStoreFilters.value ? `筛选结果 ${number(data.value?.total || 0)} 条` : `共 ${number(data.value?.total || 0)} 条`)
...
const snapshotLabel = computed(() => {
  if (!overviewData.value?.latest_snapshot_at) return "暂无库存快照"
  const stamp = overviewData.value.latest_snapshot_at.replace("T", " ").replace(/\+.*$/, "")
  return `${overviewData.value.latest_business_day || "--"} · ${stamp}`
})
const snapshotAgeLabel = computed(() => {
  const age = syncStatus.value?.snapshot_age_minutes ?? overviewData.value?.snapshot_age_minutes
...
const collectionStatusLabel = computed(() => {
  ...
  if (status === "success" && syncStatus.value?.freshness_status === "stale") return "快照已过期"
...
async function loadStore(): Promise<void> {
  ...
  error.value = ""
  try {
    const management = await fetchInventoryManagement({
      storeId,
      query: appliedStoreFilters.value.query || undefined,
      series: appliedStoreFilters.value.series || undefined,
      specification: appliedStoreFilters.value.specification || undefined,
      size: appliedStoreFilters.value.size || undefined,
      stockStatus: appliedStoreFilters.value.stockStatus || undefined,
      page: page.value,
      pageSize: pageSize.value,
    })
    if (request !== storeRequest || storeId !== selectedStoreId.value) return
    data.value = management
    if (!hasAppliedStoreFilters.value && page.value === 1) {
      overviewData.value = management
      overviewError.value = ""
    }
  } catch (requestError) {
...
async function loadBaseAnalysis(): Promise<void> {
  const request = ++analysisRequest
  const storeId = selectedStoreId.value
  analysisLoading.value = true
  analysisError.value = ""
  try {
    const result = await fetchInventoryAnalysis({ storeId })
    if (request !== analysisRequest || storeId !== selectedStoreId.value) return
    baseAnalysis.value = result
    if (!appliedAnalysisQuery.value && !appliedAnalysisStatus.value) companyAnalysis.value = result
    if (!appliedPackageQuery.value) packageAnalysis.value = result
  } catch (requestError) {
    if (request === analysisRequest) analysisError.value = requestError instanceof Error ? requestError.message : "库存分析数据暂不可用"
  } finally {
    if (request === analysisRequest) analysisLoading.value = false
  }
}

async function loadAnalysis(view: "company" | "packages"): Promise<void> {
  const request = ++analysisRequest
  const storeId = selectedStoreId.value
  analysisLoading.value = true
  analysisError.value = ""
  try {
    const result = await fetchInventoryAnalysis({
      storeId,
      query: view === "company" ? appliedAnalysisQuery.value || undefined : appliedPackageQuery.value || undefined,
      stockStatus: view === "company" ? appliedAnalysisStatus.value || undefined : undefined,
    })
    if (request !== analysisRequest || storeId !== selectedStoreId.value) return
    if (view === "company") companyAnalysis.value = result
    else packageAnalysis.value = result
  } catch (requestError) {
    if (request === analysisRequest) analysisError.value = requestError instanceof Error ? requestError.message : "库存分析数据暂不可用"
  } finally {
    if (request === analysisRequest) analysisLoading.value = false
...
async function applyAnalysisFilters(): Promise<void> {
  if (activeView.value === "company") {
    appliedAnalysisQuery.value = analysisQuery.value.trim()
    appliedAnalysisStatus.value = analysisStatus.value
    if (!appliedAnalysisQuery.value && !appliedAnalysisStatus.value && baseAnalysis.value) companyAnalysis.value = baseAnalysis.value
    else await loadAnalysis("company")
  } else {
    appliedPackageQuery.value = packageQuery.value.trim()
    if (!appliedPackageQuery.value && baseAnalysis.value) packageAnalysis.value = baseAnalysis.value
    else await loadAnalysis("packages")
  }
}

async function clearAnalysisFilters(): Promise<void> {
  if (activeView.value === "company") {
    analysisQuery.value = analysisStatus.value = ""
  } else {
    packageQuery.value = ""
  }
...
async function applyFilters(): Promise<void> {
  appliedStoreFilters.value = { query: query.value.trim(), series: series.value, specification: specification.value, size: size.value, stockStatus: stockStatus.value }
  page.value = 1
  await loadStore()
}

async function clearStoreFilters(): Promise<void> {
  query.value = series.value = specification.value = size.value = stockStatus.value = ""
  await applyFilters()
}

async function focusStockStatus(status: string): Promise<void> {
  activeView.value = "store"
  query.value = series.value = specification.value = size.value = ""
  stockStatus.value = status
  await applyFilters()
  await nextTick()
  storePanel.value?.scrollIntoView({ behavior: "smooth", block: "start" })
}

function selectView(view: "store" | "company" | "packages"): void {
  activeView.value = view
  analysisError.value = ""
  if (view === "company" && (appliedAnalysisQuery.value || appliedAnalysisStatus.value) && companyAnalysis.value?.latest_snapshot_at !== baseAnalysis.value?.latest_snapshot_at) void loadAnalysis("company")
  if (view === "packages" && appliedPackageQuery.value && packageAnalysis.value?.latest_snapshot_at !== baseAnalysis.value?.latest_snapshot_at) void loadAnalysis("packages")
}

async function loadSyncStatus(refreshOnChange = true): Promise<void> {
  const request = ++statusRequest
  const storeId = selectedStoreId.value
  try {
    const result = await fetchInventorySyncStatus(storeId)
    if (request !== statusRequest || storeId !== selectedStoreId.value) return
    const snapshotChanged = syncStatus.value?.latest_snapshot_at && result.latest_snapshot_at !== syncStatus.value.latest_snapshot_at
    syncStatus.value = result
    statusError.value = ""
    if (snapshotChanged && refreshOnChange && !syncing.value) void refreshData(false)
  } catch (requestError) {
    if (request === statusRequest) statusError.value = requestError instanceof Error ? requestError.message : "采集状态暂不可用"
  }
}

async function refreshData(includeStatus = true): Promise<void> {
  const analysisRefresh = loadBaseAnalysis().then(async () => {
    if (activeView.value === "company" && (appliedAnalysisQuery.value || appliedAnalysisStatus.value)) await loadAnalysis("company")
    if (activeView.value === "packages" && appliedPackageQuery.value) await loadAnalysis("packages")
  })
  const requests: Promise<void>[] = [loadStore(), analysisRefresh]
  if (hasAppliedStoreFilters.value || page.value !== 1) {
    const storeId = selectedStoreId.value
    requests.push(fetchInventoryManagement({ storeId }).then(result => {
      if (storeId === selectedStoreId.value) {
        overviewData.value = result
        overviewError.value = ""
      }
    }).catch(() => {
      if (storeId === selectedStoreId.value) overviewError.value = "基准统计刷新失败，仍显示上次统计"
    }))
  }
  if (includeStatus) requests.push(loadSyncStatus(false))
...
async function syncNow(): Promise<void> {
  ...
  try {
    ...
    syncNoticeTone.value = "error"
    syncNotice.value = requestError instanceof Error ? requestError.message : "库存同步失败"
  } finally {
    syncing.value = false
  }
}
...
watch(selectedStoreId, () => {
  ...
  analysisRequest++
  statusRequest++
  data.value = overviewData.value = null
  baseAnalysis.value = companyAnalysis.value = packageAnalysis.value = null
  syncStatus.value = null
  overviewError.value = analysisError.value = statusError.value = ""
  query.value = series.value = specification.value = size.value = stockStatus.value = ""
  analysisQuery.value = analysisStatus.value = packageQuery.value = ""
  appliedAnalysisQuery.value = appliedAnalysisStatus.value = appliedPackageQuery.value = ""
  appliedStoreFilters.value = { query: "", series: "", specification: "", size: "", stockStatus: "" }
  page.value = 1
  void refreshData()
})
watch(pageSize, () => { page.value = 1; void loadStore() })
...
onMounted(() => {
  void (async () => {
    try {
      ...
      error.value = requestError instanceof Error ? requestError.message : "店铺列表暂不可用"
    }
  })()
  autoRefreshTimer = setInterval(() => {
    if (!syncing.value) void loadSyncStatus()
  }, 60_000)
})
onBeforeUnmount(() => {
...
<template>
  <section class="module-hero inventory-hero">
    <div class="inventory-hero-copy">
      <h2>库存管理 <span class="inventory-live-badge" :class="{ 'is-pending': !syncStatus?.configured }"><i></i>{{ syncStatus?.configured ? "自动采集" : "等待配置" }}</span></h2>
      <span>查看店铺编码、公司货品与组合关系。库存基于最近一次吉客云快照，未匹配不等于零库存。</span>
      <div class="inventory-hero-meta"><span><Database :size="13" />{{ snapshotLabel }}</span><span><Clock3 :size="13" />{{ snapshotAgeLabel }}</span></div>
    </div>
    <div class="hero-actions">
      <button class="capture-refresh secondary-action" :disabled="loading || syncing" title="重新读取库存列表和分析，不触发采集" @click="refreshData()"><RefreshCw :size="16" :class="{ spinning: loading || analysisLoading }" />刷新视图</button>
  ...
  <section v-else-if="error && !data" class="error-panel"><PackageSearch :size="22" /><div><strong>库存列表不可用</strong><p>{{ error }}</p></div><button @click="refreshData()">重试</button></section>
  <template v-else-if="data">
    ...
    <div v-if="error" class="inventory-inline-error" role="alert">库存列表刷新失败，仍显示上次结果：{{ error }} <button @click="loadStore">重试列表</button></div>
    <div v-if="overviewError" class="inventory-inline-error" role="alert">{{ overviewError }} <button @click="refreshData()">重试概览</button></div>
    <section class="inventory-metrics" aria-label="店铺商品库存概览">
      <div class="inventory-metric"><span>商品映射</span><strong>{{ number(summary.catalog_count || 0) }}</strong><small>店铺商品编码</small></div>
      <button class="inventory-metric" type="button" title="查看有库存的商品" @click="focusStockStatus('有库存')"><span>有库存</span><strong>{{ number(summary.in_stock_count || 0) }}</strong><small>查看明细 <ChevronRight :size="12" /></small></button>
      <button class="inventory-metric is-risk" type="button" title="查看零库存商品" @click="focusStockStatus('零库存')"><span>零库存</span><strong>{{ number(summary.zero_stock_count || 0) }}</strong><small>查看明细 <ChevronRight :size="12" /></small></button>
      <button class="inventory-metric is-warning" type="button" title="查看低库存商品" @click="focusStockStatus('低库存')"><span>低库存</span><strong>{{ number(summary.low_stock_count || 0) }}</strong><small>可用量 1–10 <ChevronRight :size="12" /></small></button>
      <button class="inventory-metric" type="button" title="查看未匹配库存快照的商品" @click="focusStockStatus('未匹配库存')"><span>未匹配快照</span><strong>{{ number(summary.unmatched_count || 0) }}</strong><small>不是零库存 <ChevronRight :size="12" /></small></button>
    </section>

    <section class="panel inventory-status-bar">
      <div class="inventory-collection-state" :class="`state-${syncStatus?.status || 'unknown'}`"><div class="inventory-state-heading"><i></i><strong>{{ statusError ? '采集状态暂不可用' : collectionStatusLabel }}</strong></div><span>{{ statusError || collectionStatusDetail }}</span></div>
      <div class="inventory-refresh-plan"><strong>{{ refreshPlanLabel }}</strong><span>下次检查：{{ nextRefreshLabel }}</span><small>货品主档与组合关系每日同步 · 页面每分钟检查状态</small></div>
    </section>

    <section class="inventory-view-tabs" role="tablist" aria-label="库存分析视图">
      <button type="button" role="tab" :aria-selected="activeView === 'store'" :class="{ active: activeView === 'store' }" @click="selectView('store')">店铺商品 <small>{{ number(overviewData?.total || 0) }}</small></button>
      <button type="button" role="tab" :aria-selected="activeView === 'company'" :class="{ active: activeView === 'company' }" @click="selectView('company')">公司货品 <small>{{ number(baseAnalysis?.company_summary.sku_count || 0) }}</small></button>
      <button type="button" role="tab" :aria-selected="activeView === 'packages'" :class="{ active: activeView === 'packages' }" @click="selectView('packages')">组合货品 <small>{{ number(metric(baseAnalysis?.package_summary.package_count)) }}</small></button>
    </section>

    <section v-if="activeView === 'store'" ref="storePanel" class="panel inventory-panel">
      <div class="panel-heading"><div><p>店铺商品</p><h2>库存明细</h2></div><strong class="inventory-total">{{ storeResultLabel }} <LoaderCircle v-if="loading" :size="13" class="spinning" /></strong></div>
      <div class="inventory-filters">
        ...
        <button v-if="hasAppliedStoreFilters" class="inventory-clear" type="button" :disabled="loading" @click="clearStoreFilters">清除筛选</button>
      </div>
    ...
    <section v-else class="inventory-analysis-stack">
      <section class="inventory-analysis-toolbar panel">
        ...
        <div class="inventory-analysis-actions">
          ...
          <label class="inventory-analysis-search"><span>查询货品 / 编码</span><input v-if="activeView === 'company'" v-model="analysisQuery" placeholder="如 Z27L42 或 大鱼" @keyup.enter="applyAnalysisFilters" /><input v-else v-model="packageQuery" placeholder="组合编码或名称" @keyup.enter="applyAnalysisFilters" /></label>
          <button class="capture-refresh" :disabled="analysisLoading" @click="applyAnalysisFilters"><Search :size="15" />查询</button>
          <button v-if="activeView === 'company' ? appliedAnalysisQuery || appliedAnalysisStatus : appliedPackageQuery" class="inventory-clear" type="button" :disabled="analysisLoading" @click="clearAnalysisFilters">清除筛选</button>
        </div>
      </section>
      <div v-if="analysisError" class="inventory-inline-error" role="alert">分析刷新失败，{{ analysis ? '仍显示上次结果' : '暂无可展示的结果' }}：{{ analysisError }} <button @click="activeView === 'company' ? loadAnalysis('company') : loadAnalysis('packages')">重试分析</button></div>
      ...
      <template v-if="analysis">
        <section v-if="activeView === 'company'" class="inventory-company-layout">
          ...
          <section class="panel inventory-panel">
            <div class="panel-heading"><div><p>公司货品明细</p><h2>货品编码 · SKU · 可用库存</h2></div><strong class="inventory-total">共 {{ number(analysis.company_summary.sku_count || 0) }} 条{{ analysis.company_summary.sku_count > analysis.company_rows.length ? ` · 显示前 ${analysis.company_rows.length} 条` : '' }} <LoaderCircle v-if="analysisLoading" :size="13" class="spinning" /></strong></div>
        ...
        <section v-else class="panel inventory-panel">
          <div class="panel-heading"><div><p>组合货品清单</p><h2>组合编码 · 组成 SKU · 单套用量</h2></div><strong class="inventory-total">共 {{ number(metric(analysis.package_summary.package_count)) }} 条{{ metric(analysis.package_summary.package_count) > analysis.package_rows.length ? ` · 显示前 ${analysis.package_rows.length} 条` : '' }} <LoaderCircle v-if="analysisLoading" :size="13" class="spinning" /></strong></div>
...
.inventory-status-bar span, .inventory-status-bar small { margin-top: 3px; color: var(--text-muted); font-size: 12px; }
...
.inventory-collection-state strong { font-size: 12px; }
.inventory-collection-state span, .inventory-collection-state small { color: var(--text-muted); font-size: 12px; line-height: 1.45; }
...
.inventory-view-tabs small { color: var(--accent); font-size: 12px; }
...
.inventory-analysis-toolbar p, .warehouse-summary-panel p { margin: 0 0 4px; color: var(--text-muted); font-size: 12px; }
...
.inventory-analysis-actions label > span { color: var(--text-muted); font-size: 12px; }
...
.warehouse-grid article > div:first-child { display: grid; gap: 3px; }.warehouse-grid span, .warehouse-grid small { color: var(--text-muted); font-size: 12px; }
.warehouse-status { display: flex; flex-wrap: wrap; gap: 5px; }.warehouse-status em { border-radius: 4px; padding: 3px 5px; font-style: normal; font-size: 12px; }.warehouse-status .is-good { color: #19734a; background: #eff9f2; }.warehouse-status .is-low { color: #956a1d; background: #fff9e9; }.warehouse-status .is-zero { color: #b24f4f; background: #fff3f3; }
.mapping-pill { display: inline-flex; border-radius: 4px; padding: 3px 6px; font-size: 12px; }.mapping-pill.mapped { color: #19734a; background: #eff9f2; }.mapping-pill.unmapped { color: #956a1d; background: #fff9e9; }
.package-explanation { display: flex; align-items: center; gap: 8px; margin: 0 16px 12px; border: 1px solid #e3eee7; border-radius: 6px; padding: 9px 10px; color: #5d7466; background: #f7fbf8; font-size: 12px; line-height: 1.5; }.package-explanation svg { flex: 0 0 auto; color: var(--accent); }
.inventory-warning-list { display: grid; gap: 5px; margin: 0 16px 12px; }.inventory-warning-list span { border-radius: 5px; padding: 7px 9px; color: #956a1d; background: #fff9e9; font-size: 12px; }
.package-metrics { margin: 0 16px 12px; }.component-list { color: #536d5f; font-size: 12px; }.muted-text { color: var(--text-subtle); font-size: 12px; }
...
.inventory-filters label > span { color: var(--text-muted); font-size: 12px; }
...
.inventory-table th { padding: 11px 12px; color: var(--text-muted); background: #f7faf8; font-size: 12px; font-weight: 650; text-align: left; white-space: nowrap; }
...
.inventory-table code { color: #2e654c; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size: 12px; }
.inventory-table td small { display: block; margin-top: 4px; color: var(--warning); font-size: 12px; }
.inventory-status { display: inline-flex; min-width: 60px; justify-content: center; border: 1px solid; border-radius: 3px; padding: 3px 6px; font-size: 12px; }
...
.inventory-pagination { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 12px 16px; color: var(--text-muted); font-size: 12px; }
...
.inventory-pagination button { display: inline-flex; align-items: center; gap: 4px; min-height: 29px; border: 1px solid var(--line); border-radius: 4px; padding: 0 8px; color: #5c7166; background: #fff; font-size: 12px; }
...
.inventory-eyebrow { display: flex; align-items: center; gap: 9px; color: #718178; font-size: 12px; font-weight: 650; }
.inventory-live-badge { display: inline-flex; align-items: center; gap: 5px; border: 1px solid #bfe3d0; border-radius: 999px; padding: 3px 8px; color: #18744c; background: #effaf4; font-size: 12px; font-weight: 600; }
...
.inventory-hero-meta span { display: inline-flex; align-items: center; gap: 5px; border: 1px solid #e1ebe5; border-radius: 4px; padding: 5px 8px; color: #597267; background: #f7fbf8; font-size: 12px; }
...
.inventory-stat-card > span { color: #718178; font-size: 12px; font-weight: 600; }
.inventory-stat-card > strong { margin-top: 5px; color: #193b2c; font-size: 29px; letter-spacing: 0; }
.inventory-stat-card > small { color: #7b8c83; font-size: 12px; }
...
.inventory-snapshot-card p { margin: 0 0 4px; color: #718178; font-size: 12px; }
.inventory-snapshot-card strong { color: #203b2c; font-size: 14px; line-height: 1.35; }
.inventory-snapshot-card span:not(.inventory-status-icon) { margin-top: 4px; color: #687d71; font-size: 12px; }
...
.inventory-collection-state span { margin-top: 0; color: #5f7468; font-size: 12px; line-height: 1.45; }
.inventory-collection-state small { display: inline-flex; align-items: center; gap: 4px; margin-top: 2px; color: #72857b; font-size: 12px; }
...
.inventory-plan-heading { display: inline-flex; align-items: center; gap: 6px; color: #23825a; font-size: 12px; font-weight: 650; }
.inventory-refresh-plan > strong { color: #294a3a; font-size: 12px; }
.inventory-refresh-plan > span { color: #5f7468; font-size: 12px; }
.inventory-refresh-plan > small { margin-top: 3px; color: #82928a; font-size: 12px; line-height: 1.4; }
...
/* Keep the work surface compact; the risk counts are direct filters, not decorative cards. */
.inventory-hero { min-height: 0; padding: 18px 22px; background: #fff; }
.inventory-hero-copy { gap: 6px; }
.inventory-hero h2 { display: flex; align-items: center; flex-wrap: wrap; gap: 10px; font-size: 23px; }
.inventory-live-badge { vertical-align: middle; }
.inventory-hero-copy > span { max-width: 72ch; font-size: 12px; }
.inventory-hero-meta { margin-top: 0; }
.inventory-hero-meta span { border: 0; padding: 0; background: transparent; font-size: 12px; }
.inventory-metrics { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 0; margin: 10px 0; border: 1px solid var(--line); border-radius: 6px; background: #fff; }
.inventory-metric { display: grid; align-content: center; justify-items: start; gap: 3px; min-width: 0; min-height: 83px; border: 0; border-right: 1px solid var(--line); padding: 11px 16px; color: var(--text); background: transparent; text-align: left; font: inherit; }
.inventory-metric:last-child { border-right: 0; }
.inventory-metric:is(button) { cursor: pointer; transition: background-color .16s ease-out; }
.inventory-metric:is(button):hover { background: #f4faf7; }
.inventory-metric:is(button):focus-visible, .inventory-view-tabs button:focus-visible, .inventory-clear:focus-visible { outline: 2px solid var(--accent); outline-offset: -3px; }
.inventory-metric > span { color: var(--text-muted); font-size: 12px; }
.inventory-metric > strong { color: #203b2c; font-size: 24px; line-height: 1.1; font-variant-numeric: tabular-nums; }
.inventory-metric.is-risk > strong { color: #ad433e; }
.inventory-metric.is-warning > strong { color: #916118; }
.inventory-metric > small { display: flex; align-items: center; gap: 3px; color: var(--text-muted); font-size: 12px; }
.inventory-status-bar { display: grid; grid-template-columns: minmax(0, 1.3fr) minmax(0, 1fr); min-height: 0; margin-bottom: 10px; padding: 0; }
.inventory-collection-state, .inventory-refresh-plan { padding: 10px 16px; }
.inventory-collection-state { border: 0; border-right: 1px solid var(--line); }
.inventory-status-bar > .inventory-refresh-plan { display: grid; align-content: center; gap: 2px; }
.inventory-view-tabs { width: fit-content; max-width: 100%; margin-bottom: 10px; }
.inventory-view-tabs button { white-space: nowrap; }
.inventory-total { display: inline-flex; align-items: center; gap: 6px; color: #436c55; font-size: 12px; white-space: normal; text-align: right; }
.inventory-filters { gap: 8px; }
.inventory-filters label { min-width: 108px; }
.inventory-filters select, .inventory-filters input, .inventory-analysis-actions select, .inventory-analysis-actions input { max-width: 100%; }
.inventory-clear { min-height: 34px; border: 1px solid var(--line); border-radius: 4px; padding: 0 11px; color: #456656; background: #fff; font-size: 12px; cursor: pointer; }
.inventory-clear:hover { background: #f4faf7; }
.inventory-clear:disabled { opacity: .5; cursor: not-allowed; }
.inventory-inline-error { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin: 8px 0; border: 1px solid #edc7c7; border-radius: 5px; padding: 10px 12px; color: #9c3737; background: #fff7f6; font-size: 12px; }
.inventory-inline-error button { flex: 0 0 auto; border: 0; padding: 4px; color: inherit; background: transparent; font-weight: 650; cursor: pointer; }
.inventory-table th, .inventory-table td { font-variant-numeric: tabular-nums; }
.inventory-table-wrap:focus-within { outline-color: var(--accent); }
.inventory-panel { scroll-margin-top: 112px; }
@media (max-width: 900px) {
  .inventory-metrics { grid-template-columns: repeat(3, minmax(0, 1fr)); }
  .inventory-metric:nth-child(3) { border-right: 0; }
  .inventory-metric:nth-child(n + 4) { border-top: 1px solid var(--line); }
  .inventory-status-bar { grid-template-columns: 1fr; }
  .inventory-collection-state { border-right: 0; border-bottom: 1px solid var(--line); }
  .inventory-refresh-plan { grid-column: auto; border-top: 0; }
}
@media (max-width: 760px) {
  .inventory-hero { padding: 16px; }
  .inventory-hero h2 { font-size: 21px; }
  .inventory-hero .hero-actions { display: flex; }
  .inventory-metrics { grid-template-columns: repeat(2, minmax(0, 1fr)); }
  .inventory-metric { min-height: 72px; padding: 9px 12px; }
  .inventory-metric:nth-child(3) { border-right: 1px solid var(--line); }
  .inventory-metric:nth-child(even) { border-right: 0; }
  .inventory-metric:nth-child(n + 3) { border-top: 1px solid var(--line); }
  .inventory-status-bar > div { min-height: 0; }
  .inventory-view-tabs { width: 100%; }
  .inventory-filters label, .inventory-analysis-actions label { min-width: 0; flex: 1 1 135px; }
  .inventory-filters .inventory-search, .inventory-analysis-search { flex-basis: 100%; }
  .inventory-filters > button, .inventory-analysis-actions > button { flex: 1 1 auto; }
  .inventory-pagination { align-items: stretch; }
  .inventory-pagination > div { flex-wrap: wrap; }
  .inventory-inline-error { align-items: flex-start; }
}
</style>
