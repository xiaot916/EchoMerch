...
from app.core.local_database import PROMOTION_CROWD_METRICS


ENDPOINT_KEY = "alimama.report.query.crowd"
PARSER_VERSION = "alimama-crowd-v1"


class AlimamaCrowdPayloadError(ValueError):
    pass


@dataclass(frozen=True)
class PromotionCrowdRow:
    scene_name: str
    campaign_id: str
    campaign_name: str
    adgroup_id: str
...
@dataclass(frozen=True)
class ParsedAlimamaCrowdReport:
    platform_store_id: str
    business_day: date
    rows: tuple[PromotionCrowdRow, ...]
    response_code: int
    source_sha256: str
    source_bytes: int
    ...
    def metric_count(self) -> int:
        return len(self.rows) * len(PROMOTION_CROWD_METRICS)
...
) -> ParsedAlimamaCrowdReport:
    raw = path.read_bytes()
    try:
        payload = json.loads(raw.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise AlimamaCrowdPayloadError("The Alimama crowd response is not valid JSON.") from exc
...
def parse_payload(
    ...
    if not isinstance(payload, dict):
        raise AlimamaCrowdPayloadError("The Alimama crowd response must be an object.")
    ...
    rows_by_key: dict[tuple[str, str, str, str], PromotionCrowdRow] = {}
    for page_payload in source_payloads:
        response_code = _response_code(page_payload)
        if response_code != 0:
            raise AlimamaCrowdPayloadError(
                f"The Alimama crowd response failed with code {response_code}."
            )
        data = page_payload.get("data")
        rows_value = data.get("list") if isinstance(data, dict) else None
        if not isinstance(rows_value, list):
            raise AlimamaCrowdPayloadError("The Alimama crowd response has no data.list.")
        for raw_row in rows_value:
            ...
            crowd_id = _text(raw_row.get("bpCrowdId") or raw_row.get("crowdId")) or ""
            subject_id = _text(
                raw_row.get("promotionId") or raw_row.get("blackCreativePromotionId")
            ) or ""
            if not crowd_id:
                continue
            key = (campaign_id, adgroup_id, crowd_id, subject_id)
            rows_by_key[key] = PromotionCrowdRow(
                ...
                crowd_id=crowd_id,
                crowd_name=_text(raw_row.get("crowdName")) or "",
                ...
                metrics=tuple(
                    ...
                    for column in PROMOTION_CROWD_METRICS
                ),
            )

    return ParsedAlimamaCrowdReport(
