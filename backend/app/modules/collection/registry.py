...
class CollectionDataset:
    ...
    # Registry platform code ("sycm", "alimama", "cps", "databank",
    # "brandsearch", "utry") used to resolve the session via the single
    # SessionProviderRegistry instead of importing a per-platform
    # resolve_*_runtime_context.  Leave "" for datasets that need no platform
    # session (e.g. internal store-only jobs).
    platform_code: str = ""
...
COLLECTION_DATASETS: tuple[CollectionDataset, ...] = (
    ...
    CollectionDataset(
        ...
        ("store_daily_traffic_source",), "多层级来源流量与成交贡献。",
        platform_code="sycm",
    ),
    CollectionDataset(
    ...
    CollectionDataset(
        "cps_items", "淘宝客 CPS 商品明细", "推广", ("store_daily_cps_items",),
        ("cps_items",), "淘宝客商品级点击、付款、佣金与结算明细（支持近13个月回溯）。",
        platform_code="cps",
    ),
    CollectionDataset(
    ...
    CollectionDataset(
        "sycm_home_board", "SYCM 首页看板", "经营", ("store_daily_sycm_home_board",),
        ("sycm_home_board",), "首页体验分与主营类目；增长因子、层级尚无可用字段。",
        allow_no_data=True,
        platform_code="sycm",
    ),
)
