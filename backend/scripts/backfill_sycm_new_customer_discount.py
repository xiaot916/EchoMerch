...
from scripts.fetch_sycm_new_customer_discount import (  # noqa: E402
    FetchResult,
    fetch_sycm_new_customer_discount,
)
...
DEFAULT_OUTPUT_DIR = (
    ...
    / "raw_responses"
    / "sycm_new_customer_discount_backfill"
)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill SYCM new customer discount data day by day."
    )
    ...
    log_path = args.output_dir / f"backfill_new_customer_discount_{_timestamp()}.jsonl"
    ...
    run_id = crawl_runs.create_run(
        store_id=args.store_id,
        task_type="sycm_new_customer_discount",
    ...
    try:
        for index, day in enumerate(days, start=1):
            ...
            try:
                ...
                result = warehouse.ingest_sycm_new_customer_discount(
...
def _existing_days(database: LocalDatabase, store_id: int) -> set[date]:
    ...
    try:
        rows = conn.execute(
            f"select {q(BUSINESS_DAY)} from "
            f"store_daily_new_customer_discount_overviews "
            f"where {q(STORE_ID)}=? "
...
def _response_path(output_dir: Path, day: date) -> Path:
    return (
        ...
        / f"{day.year:04d}"
        / f"{day.month:02d}"
        / f"sycm_new_customer_discount_{day.strftime('%Y%m%d')}.json"
    )
...
if __name__ == "__main__":
    try:
        ...
        print(f"New customer discount backfill failed: {exc}", file=sys.stderr)
