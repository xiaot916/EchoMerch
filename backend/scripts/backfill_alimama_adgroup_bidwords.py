...
DEFAULT_OUTPUT_DIR = (
    ...
    / "raw_responses"
    / "alimama_adgroup_bidword_backfill"
)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill Alimama promotion adgroup or bidword daily facts."
    )
    parser.add_argument("--detail-type", choices=("adgroup", "bidword"), required=True)
    ...
    table = (
        PROMOTION_ADGROUP_TABLE
        if args.detail_type == "adgroup"
        else PROMOTION_BIDWORD_TABLE
    )
    ...
    runtime = (
        resolve_alimama_runtime_context(
            ...
            report_home_url=alimama_report_home(args.detail_type),
    ...
    run_id = runs.create_run(
        store_id=args.store_id,
        task_type=f"alimama_{args.detail_type}s",
    ...
    try:
        for index, business_day in enumerate(days, start=1):
            if not args.refresh_existing and business_day in existing:
                ...
                print(f"[{index}/{len(days)}] {business_day} skipped existing")
                continue
            try:
                response_path, pages, source_rows = _fetch_day(
                    business_day, args, runtime, args.output_dir
                )
                ...
                ingest = (
                    warehouse.ingest_alimama_adgroup_report
                    if args.detail_type == "adgroup"
                    else warehouse.ingest_alimama_bidword_report
                )
                result = ingest(
                ...
                print(
                    f"[{index}/{len(days)}] {business_day} ingested "
                    f"{source_rows} rows ({pages} pages)"
                )
                ...
                _log(
                    log_path,
                    {"day": business_day.isoformat(), "status": "failed", "error": str(exc)},
                )
                print(f"[{index}/{len(days)}] {business_day} failed: {exc}")
...
def _fetch_day(
    ...
    config = REPORT_CONFIG[args.detail_type]
    ...
    for page_number in range(1, args.max_pages + 1):
        ...
        if not 200 <= status < 300 or code != 0:
            raise RuntimeError(
                f"{args.detail_type} page fetch failed: HTTP {status}, "
        ...
        if page_number == 1 and not rows:
            for retry_number in range(1, args.empty_page_retries + 1):
                ...
                if not 200 <= status < 300 or code != 0:
                    raise RuntimeError(
                        f"{args.detail_type} page retry failed: HTTP {status}, "
                        f"code {code}, message {message}"
                    )
                payload = json.loads(page_path.read_text(encoding="utf-8"))
                data = payload.get("data") if isinstance(payload, dict) else None
                ...
                if rows:
                    break
                print(
                    f"{business_day.isoformat()} empty {args.detail_type} response "
                    f"retry {retry_number}/{args.empty_page_retries}"
                )
        pages.append(payload)
        rows_total += len(rows)
        count = _integer(data.get("count")) if isinstance(data, dict) else None
        count_finished = (
            not config.get("by_page_without_count", False)
            and count is not None
            and offset + len(rows) >= count
        )
        if not rows or len(rows) < args.page_size or count_finished:
            break
        offset += len(rows)
        if args.page_sleep:
            time.sleep(args.page_sleep)
    else:
        raise RuntimeError(f"{args.detail_type} pagination exceeded --max-pages")
...
def _existing_days(database: LocalDatabase, table: str, store_id: int) -> set[date]:
    with database.connect() as conn:
        rows = conn.execute(
            f"select {q(BUSINESS_DAY)} from {table} "
...
if __name__ == "__main__":
    try:
        ...
        print(f"Alimama detail backfill failed: {exc}", file=sys.stderr)
