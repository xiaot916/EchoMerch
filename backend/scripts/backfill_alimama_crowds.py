...
from app.core.local_database import BUSINESS_DAY, PROMOTION_CROWD_TABLE, STORE_ID, LocalDatabase, q  # noqa: E402
...
DEFAULT_OUTPUT_DIR = (
    Path(settings.local_database_path).parent / "raw_responses" / "alimama_crowd_backfill"
)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill Alimama crowd-level promotion metrics one day at a time."
    )
    ...
    log_path = args.output_dir / f"backfill_store_daily_promotion_crowds_{_stamp()}.jsonl"
    runtime = (
        resolve_alimama_runtime_context(
            ...
            report_home_url=alimama_report_home("crowd"),
        ...
        for index, business_day in enumerate(days, start=1):
            ...
            try:
                ...
                print(
                    ...
                    f"{source_rows} crowd rows ({pages} pages)"
                )
...
def _fetch_day(
    ...
    for page_number in range(1, args.max_pages + 1):
        ...
        if page_number == 1 and not rows:
            for retry_number in range(1, args.empty_page_retries + 1):
                ...
                if rows:
                    break
                print(
                    f"{business_day.isoformat()} empty crowd response "
        ...
        raise RuntimeError("crowd pagination exceeded --max-pages")

    output = args.output_dir / f"crowd_{business_day.isoformat()}.json"
    output.write_text(
...
def _fetch_page(
    *,
    ...
    if not 200 <= status < 300 or code != 0:
        raise RuntimeError(
            f"crowd page fetch failed: HTTP {status}, code {code}, message {message}"
        )
...
def _existing_days(database: LocalDatabase, store_id: int) -> set[date]:
    ...
    try:
        rows = conn.execute(
            f"select {q(BUSINESS_DAY)} from {PROMOTION_CROWD_TABLE} "
...
if __name__ == "__main__":
    try:
        ...
        print(f"Alimama crowd backfill failed: {exc}", file=sys.stderr)
