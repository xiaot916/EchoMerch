...
def fetch_sycm_shopping_gold(
    *,
    ...
    if token:
        params["token"] = token
    request = Request(
        "https://sycm.taobao.com/xsite/rc/getRcOverall.json?"
        f"{urlencode(params)}",
        headers={
            "accept": "*/*",
            ...
            "cookie": cookie,
            "onetrace-card-id": "promotionSalesPage.promotionMethodPage.promotionGouwujinPage.promotionGouwujinOverview",
            ...
            "sycm-query": "activeKey=promotionMethod",
            "sycm-referer": "/xsite/promotion/promotion/sales",
            "user-agent": (
    ...
    return FetchResult(status, code, message, str(output), len(output_body))


def main() -> int:
    parser = argparse.ArgumentParser(description="Fetch one day of SYCM shopping-gold data.")
    ...
    result = fetch_sycm_shopping_gold(
        day=args.day,
        output=args.output,
...
def _response_code_and_message(payload: object) -> tuple[int | None, str | None]:
    ...
    if isinstance(content, dict):
        ...
        message = content.get("message") or content.get("msg")
    else:
...
if __name__ == "__main__":
    try:
        ...
        print(f"Shopping-gold fetch failed: {exc}", file=sys.stderr)
