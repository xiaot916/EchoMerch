...
from scripts.fetch_sycm_shopping_gold import (  # noqa: E402
    fetch_sycm_shopping_gold,
)


DEFAULT_STORE_NAME = "碧芭宝贝旗舰店"
DEFAULT_PLATFORM_STORE_ID = "2200573698992"
DEFAULT_OUTPUT_DIR = Path(settings.local_database_path).parent / "raw_responses" / "sycm_shopping_gold_backfill"


def main() -> int:
    parser = argparse.ArgumentParser(description="Backfill SYCM shopping-gold data day by day.")
    ...
    log_path = args.output_dir / f"backfill_store_daily_shopping_gold_{_timestamp()}.jsonl"
    ...
    run_id = crawl_runs.create_run(store_id=args.store_id, task_type="sycm_shopping_gold", start_day=args.start, end_day=args.end, mode="backfill", planned_days=len(days), log_file=log_path)
    ...
    try:
        for index, day in enumerate(days, start=1):
            ...
            if not args.refresh_existing and day in existing_days:
                ...
                continue
            try:
                fetched = fetch_sycm_shopping_gold(day=day, output=output, cookie=session.cookie_header if session else "", token=args.token, timeout=args.timeout)
                if not fetched.ok:
                    raise RuntimeError(f"fetch failed: HTTP {fetched.status}, code {fetched.code}, message {fetched.message}")
                result = warehouse.ingest_sycm_shopping_gold(source_path=output, business_day=day, store_name=args.store_name, platform_store_id=args.platform_store_id, http_status=fetched.status)
                summary["inserted"] += 1
                consecutive_failures = 0
                crawl_runs.record_day(run_id=run_id, store_id=args.store_id, business_day=day, status="ingested", metric_count=result.metric_count)
                _write_log(log_path, {"day": day.isoformat(), "status": "ingested", "fetch": asdict(fetched), "metric_count": result.metric_count})
...
def _existing_days(database: LocalDatabase, store_id: int) -> set[date]:
    ...
    try:
        rows = conn.execute(f"select {q(BUSINESS_DAY)} from store_daily_shopping_gold_overviews where {q(STORE_ID)}=?", (store_id,)).fetchall()
    finally:
        conn.close()
...
if __name__ == "__main__":
    try:
        ...
        print(f"Shopping-gold backfill failed: {exc}", file=sys.stderr)
