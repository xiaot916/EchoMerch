...
DEFAULT_OUTPUT_DIR = Path(settings.local_database_path).parent / "raw_responses" / "sycm_bybt_items_backfill"


def main() -> int:
    parser = _parser("Backfill SYCM BYBT item data day by day.")
    ...
    run_id = runs.create_run(store_id=args.store_id, task_type="sycm_bybt_items", start_day=args.start, end_day=args.end, mode="refresh" if args.refresh_existing else "backfill", planned_days=len(days), log_file=log_path)
    ...
    try:
        for index, day in enumerate(days, start=1):
            ...
            try:
                ...
                summary["inserted" if rows else "no_data"] += 1
                runs.record_day(
                    run_id=run_id,
        ...
        runs.finish_run(
            ...
            success_days=summary["inserted"] + summary["no_data"],
...
def _validate(args: argparse.Namespace) -> None:
    args.database_path, args.output_dir = args.database_path.expanduser().resolve(), args.output_dir.expanduser().resolve()
    if args.end < args.start or not 1 <= args.page_size <= 100 or args.max_pages < 1:
        raise ValueError("invalid date range, page-size, or max-pages")
...
def _fetch_day(
    ...
    for page in range(1, args.max_pages + 1):
        ...
        payloads.append(payload); total_rows += len(page_rows)
    ...
    combined = args.output_dir / f"bybt_items_{day.isoformat()}.json"
    combined.write_text(json.dumps({"pages": payloads}, ensure_ascii=False, separators=(",", ":")), encoding="utf-8")
    return combined, total_rows, len(payloads), stop_reason
...
def _page_metadata(payload: object) -> tuple[list[object], int | None]:
    def find(value: object, depth: int = 0) -> tuple[list[object], int | None]:
        ...
        for key in ("data", "list", "pageData", "rows", "items"):
            rows = value.get(key)
            if isinstance(rows, list):
                for count_key in ("recordCount", "total", "totalCount"):
                    try: return rows, int(value[count_key]) if value.get(count_key) is not None else None
    ...
    with database.connect() as conn:
        rows = conn.execute(
            ...
            where days.{q(STORE_ID)} = ?
              and runs.{q(CRAWL_TASK_TYPE)} = 'sycm_bybt_items'
...
def _overview_days(database: LocalDatabase, store_id: int, start: date, end: date) -> list[date]:
    # Item detail is an independent endpoint. It must still be collected when
    # the overview endpoint is incomplete for the same business day.
