...
DEFAULT_OUTPUT_DIR = Path(settings.local_database_path).parent / "raw_responses" / "cps_overview_backfill"


def main() -> int:
    parser = argparse.ArgumentParser(description="Backfill CPS daily overview data.")
    ...
    log_path = args.output_dir / f"backfill_store_daily_cps_{_stamp()}.jsonl"
    runtime = (
        resolve_cps_runtime_context(
    ...
    run_id = runs.create_run(store_id=args.store_id, task_type="cps_overview", start_day=args.start, end_day=args.end, mode="refresh" if args.refresh_existing else "backfill", planned_days=len(days), log_file=log_path)
    ...
    try:
        for index, day in enumerate(days, start=1):
            ...
            try:
                ...
                result = warehouse.ingest_cps_overview(source_path=output, business_day=day, store_name=args.store_name, platform_store_id=args.platform_store_id, http_status=fetched[0])
                summary["inserted"] += 1
                failures = 0
                runs.record_day(run_id=run_id, store_id=args.store_id, business_day=day, status="ingested", metric_count=result.metric_count)
                _log(log_path, {"day": day.isoformat(), "status": "ingested", "fetch": {"status": fetched[0], "code": fetched[1], "message": fetched[2], "bytes": fetched[3]}, "metric_count": result.metric_count})
...
def _existing_days(database: LocalDatabase, store_id: int) -> set[date]:
    ...
    try:
        rows = conn.execute(f"select {q(BUSINESS_DAY)} from store_daily_cps_overviews where {q(STORE_ID)}=?", (store_id,)).fetchall()
    finally:
        conn.close()
...
if __name__ == "__main__":
    try:
        ...
        print(f"CPS backfill failed: {exc}", file=sys.stderr)
