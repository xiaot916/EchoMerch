...
from scripts.fetch_sycm_customer_overview import (  # noqa: E402
    FetchResult,
    fetch_sycm_customer_overview,
)
...
DEFAULT_OUTPUT_DIR = (
    ...
    / "raw_responses"
    / "sycm_customer_overview_backfill"
)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Backfill SYCM customer overview data day by day."
    )
    ...
    log_path = args.output_dir / f"backfill_customer_overviews_{_timestamp()}.jsonl"
    ...
    run_id = crawl_runs.create_run(
        store_id=args.store_id,
        task_type="sycm_customer_overview",
    ...
    try:
        for index, day in enumerate(days, start=1):
            ...
            if not args.refresh_existing and day in existing_days:
                ...
                continue
            try:
                fetched = fetch_sycm_customer_overview(
                    day=day,
                ...
                result = warehouse.ingest_sycm_customer_overview(
...
def _existing_days(database: LocalDatabase, store_id: int) -> set[date]:
    ...
    try:
        rows = conn.execute(
            f"select {q(BUSINESS_DAY)} from store_daily_customer_overviews "
            f"where {q(STORE_ID)}=?",
            (store_id,),
        ).fetchall()
    finally:
...
def _response_path(output_dir: Path, day: date) -> Path:
    return (
        ...
        / f"{day.year:04d}"
        / f"{day.month:02d}"
        / f"sycm_customer_overview_{day.strftime('%Y%m%d')}.json"
    )
...
if __name__ == "__main__":
    try:
        ...
        print(f"Customer overview backfill failed: {exc}", file=sys.stderr)
