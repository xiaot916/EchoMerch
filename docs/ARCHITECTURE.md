...
EchoMerch is evolving into a Tmall/Qianniu operations data console. The first stable boundary is read-only: it can inspect historical data, Reqable captures, collection progress, and operation guardrails. It does not replay platform requests or execute batch mutations from HTTP routes.
...
- `modules.imports`: exposes date-level crawl task records in SQLite.
...
Crawler replay, browser automation, coupon creation, and other platform writes must be implemented as audited workers before any API endpoint can trigger them.
...
- Imports: daily crawl task history and date-level progress.
- Operations: future batch action center with preview-first safety flow.
- Tasks: capability and automation roadmap.

## Evolution Order

1. Store raw response artifacts and parser output in staging tables.
2. Build fact tables for shop daily, product daily, traffic source, and member analysis.
3. Add scheduler state for repeatable 2-3 day test windows; keep scheduling outside FastAPI request handlers.
4. Add operation templates only after preview, confirmation, idempotency, and audit are implemented.
